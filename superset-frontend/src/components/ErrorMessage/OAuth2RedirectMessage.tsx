...
export function OAuth2RedirectMessage({
  ...
  const body = (
    <p>
      {t(
        'This database uses OAuth2 for authentication. Please click the link above to grant DataHub permission to access the data. Your personal access token will be stored encrypted and used only for queries run by you.',
      )}
    </p>
  );
  const subtitle = (
